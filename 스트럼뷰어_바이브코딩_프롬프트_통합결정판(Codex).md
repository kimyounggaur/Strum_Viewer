...
- **6/8 · 12/8** : 한 박을 3등분하는 겹박자. 보통 8분음표 다운 스트로크가 3개씩 묶여 그룹을 이룸. 시작 박에 악센트가 흔함.
...
- manifest에는 id, categoryId, title, exerciseNo, originalFileName, imageSrc, width, height, timeSignature, feel, tags, recommendedBpm을 포함하라.
...
│  │  ├─ strumPatterns.generated.ts    # 스크립트 생성(이미지 메타)
...
export type Stroke = {
  // 마디 안에서의 위치(분할 격자 인덱스). subdivision=4(16분)면 0..15, 8분이면 0..7 ...
  step: number;
  dir: StrokeDir;
  accent?: boolean;       // 악센트 > (세게)
  tie?: boolean;          // 붙임줄: 새로 안 치고 이전 음을 지속
  durationSteps?: number; // 음 길이(격자 단위, 기본 1) — 점음표 등
  xNorm?: number;         // (선택) 0..1, 소스 이미지 가로상의 정확한 위치(플레이헤드 정밀 정렬용)
};

export type StrumRhythm = {
  timeSignature: [number, number];        // [4,4] / [3,4] / [6,8] / [12,8]
  beatsPerBar: number;                     // 박자표 분자 (3/4→3, 6/8→6, 12/8→12)
  subdivision: number;                     // "한 박"을 몇으로 쪼개나 (16분=4, 8분=2, 셋잇단=3, 겹박자 8분=1)
  bars: number;                            // 보통 1 (반복)
  feelTiming: 'straight' | 'swing' | 'triplet' | 'compound';
  swing?: number;                          // 0~1 (셔플 약 0.6~0.67). feelTiming='swing'일 때만
  strokes: Stroke[];                       // 리듬의 핵심

  // 이미지 위 플레이헤드 정렬용(선택). 없으면 leftPad/rightPad로 step→x 균등 매핑.
  imageLeftPad?: number;                   // 악보 시작 x (이미지 폭 대비 0..1)
  imageRightPad?: number;                  // 악보 끝 x (0..1)
};
```

> ⚠️ **컨벤션 고정**: `step`은 “분할 격자 인덱스”라는 단 하나의 규칙만 쓴다. (예: 4/4 16분 → 한 마디 16칸 0..15.) 이 규칙을 강하게 못박아 좌우/박 어긋남을 방지한다.
...
export const strumRhythms: Record<string, StrumRhythm> = {
  // ── 6/8 Ex-1 : 8분음표 다운 6개(3+3 그룹), 1·4박 악센트 (소스 확인: 가장 단순) ──
  'six-eight_ex01': {
    timeSignature: [6, 8], beatsPerBar: 6, subdivision: 1, bars: 1, feelTiming: 'compound',
    strokes: [
  ...
  // ── 12/8 Ex-1 : 8분음표 다운 12개(3×4 그룹), 박머리 악센트 ──
  'twelve-eight_ex01': {
    timeSignature: [12, 8], beatsPerBar: 12, subdivision: 1, bars: 1, feelTiming: 'compound',
    strokes: Array.from({ length: 12 }, (_, i) => ({ step: i, dir: 'down' as const, accent: i % 3 === 0 })),
  },

  // ── Slow Rock Ex-1 : 4/4, 모든 박 셋잇단(triplet) 다운 12개 ──
  'slow-rock_ex01': {
    timeSignature: [4, 4], beatsPerBar: 4, subdivision: 3, bars: 1, feelTiming: 'triplet',
    strokes: Array.from({ length: 12 }, (_, i) => ({ step: i, dir: 'down' as const, accent: i % 3 === 0 })),
  },

  // ── 3/4 Ex-1 : 한 마디 3박, 다운-업-업 + 1박 악센트 (예시·근사) ──
  'three-four_ex01': {
    timeSignature: [3, 4], beatsPerBar: 3, subdivision: 2, bars: 1, feelTiming: 'straight',
    strokes: [
  ...
  // ── Shuffle Ex-1 : 4/4 스윙, "다운 … 업-다운" 굴림 (예시·근사) ──
  'shuffle_ex01': {
    timeSignature: [4, 4], beatsPerBar: 4, subdivision: 2, bars: 1, feelTiming: 'swing', swing: 0.65,
    // 8분 격자 step 0..7. 셔플은 even step=박머리(다운), odd step=뒷박(스윙 업)
    strokes: [
      { step: 0, dir: 'down' },
      { step: 2, dir: 'down' }, { step: 3, dir: 'up' },
      { step: 4, dir: 'down' }, { step: 5, dir: 'up' },
...
> 나머지 패턴(16 Beat, Country, Calypso, Slow GoGo, 각 스타일 Ex-2~)은 [부록 B 인덱스](#부록-b-9개-카테고리--예제-인덱스-소스-폴더-그대로)와 실제 PNG를 보고 같은 형식으로 채운다.
...
| 빔 1개 / 2개 | `subdivision:2` / `4` | 8분 / 16분 격자 |
| 점음표 `.` | `durationSteps` 조정 | 더 길게 |
| 곡선 tie | `tie:true` | 새로 안 침, 지속 |
| 쉼표 | `dir:'rest'` | 무음 |
| ×머리(뮤트) | `dir:'mute'` | 짧고 둔탁 |
| `║: … :║` | `bars`·loop | 반복 |
| 셔플 “길게-짧게” | `feelTiming:'swing'`, `swing≈0.66` | 뒷박 지연 |
...
- 카드 정보: Ex 번호, 카테고리명, 박자표, 추천 BPM, 즐겨찾기 버튼, (rhythm 있으면) 작은 ▶︎ 미리듣기.
...
- subdivision 카운트 표시(별도 패널):
  ...
  - 3/4: `1 & 2 & 3 &`   ·   6/8: `1 la li 2 la li`   ·   12/8: `1 la li 2 la li 3 la li 4 la li`
  - shuffle/triplet: `1 trip let 2 trip let …`
...
- 위치 계산: 재생 시간 → 마디 내 진행률(0..1) → 화면 x. (이미지 모드는 `xNorm` 보간 또는 `imageLeftPad/Right Pad` 균등 매핑, SVG 모드는 좌표 직접.)
...
- 카운트인 중에는 `1·2·3·4` 오버레이(선택). 정지 시 플레이헤드는 시작 위치로 복귀(또는 마지막 위치 유지 옵션).
...
- 원본 파일명에서 EX01/Ex01 등 번호를 추출해 exerciseNo를 만든다.
- 이미지 width/height를 읽어 manifest에 포함한다(image-size 패키지 또는 PNG 헤더 직접 파싱).
- `src/data/strumPatterns.generated.ts`를 생성한다. 생성 데이터는 StrumPattern[] 타입을 만족해야 한다.
...
strumTypes.ts: StrumCategoryId, TimeSignature, StrumFeel, Difficulty, BpmRange, StrumCategory, StrumPattern
  (StrumPattern에는 이후 업그레이드용 optional `rhythm?: StrumRhythm` 필드도 미리 선언해 둔다. StrumRhythm/Stroke 타입도 정의).
...
기능: fit-width/fit-height/actual/zoom in·out/reset/drag pan/더블클릭 fit·actual 토글/wheel zoom(Ctrl 또는 focus 시)/로딩·실패 fallback/alt.
...
```text
src/data/strumTypes.ts에 StrokeDir, Stroke, StrumRhythm 타입(§5.3)을 추가하라. StrumPattern에 optional rhythm 필드 연결.
...
- step 격자 규칙(§5.3 ⚠️)을 한 가지로 고정. 각 패턴에 timeSignature/beatsPerBar/subdivision/feelTiming(/swing) 채움.
- 확신 없는 디테일엔 'TODO: verify' 주석. 표시는 이미지가 정답이므로 문제없음.
- pattern.id와 strumRhythms 키를 일치시켜, 로딩 시 패턴에 rhythm을 머지하라.
```

### Step U2. 시간 계산 유틸
```text
src/lib/rhythm.ts를 만들어라.
- secPerBeat(bpm) = 60/bpm.
- stepToTime(step, rhythm, bpm): step(분할 인덱스) → 마디 내 상대 시각(초).
  straight/triplet/compound는 균등 분할. swing(subdivision=2)은 짝수 step=박머리, 홀수 step=beat*swing 만큼 뒤로.
- barDuration(rhythm, bpm): 한 마디 길이(초). 겹박자(6/8,12/8)는 8분 기준 또는 "분자/3 박"으로 환산해 일관되게.
...
AudioEngine.ts: lookahead=25ms 타이머가 scheduleAheadTime=0.1s 앞을 미리 예약하는 룩어헤드 스케줄러.
  ...
  AudioContext는 사용자 제스처 후 resume. 정지 시 예약 노드 정리. setInterval 단독 금지.
...
useTransport: AudioEngine 래핑. 상태 isPlaying/bpm/loop/countIn/instrument + progress(0..1, requestAnimationFrame로 갱신) + activeStrokeIndex.
...
StrumSheet: 상세 이미지를 감싸 progress·activeStrokeIndex로 빨강 플레이헤드 세로선과 노랑 하이라이트를 오버레이.
  이미지 모드 x좌표 = stroke.xNorm 보간(없으면 rhythm.imageLeftPad/RightPad로 step→x 균등 매핑).
...
- [ ] 76개 PNG가 모두 `public/strums/{categoryId}/exNN.png`로 복사된다.
- [ ] manifest에 76개 패턴이 모두 들어가고, 원본 파일명·width·height가 보존된다.

### 기능 (Core)
- [ ] 9개 카테고리 선택 / 카테고리별 패턴 개수 표시 / 패턴 갤러리 / 검색 / 정렬
- [ ] 상세 이미지 확대·축소 / 드래그 패닝 / 이전·다음 / 키보드 단축키
- [ ] 즐겨찾기 / 최근 본 패턴 / BPM 메트로놈 / count-in·accent 옵션

### 기능 (Upgrade)
- [ ] rhythm 있는 패턴은 ▶︎ 재생 시 소리가 나고, 빨강 플레이헤드가 박자에 맞춰 이동, 현재 스트로크 하이라이트
- [ ] 다운/업/악센트/쉼/뮤트/스윙/셋잇단이 소리에 반영
...
- [ ] (Upgrade) 오디오가 룩어헤드 스케줄러로 타이밍 안정, AudioContext는 제스처 후 resume, 이탈 시 정지·정리
- [ ] (Upgrade) 표시(이미지) ↔ 소리(rhythm) 분리, 오디오 로직 src/audio/*·저장 로직 src/lib/* 격리
...
2) 소리·플레이헤드: 별도 리듬 데이터(strokes)에서 구동하라. 오디오는 룩어헤드 스케줄러로 타이밍을 안정시키고
   다운/업/악센트/스윙/셋잇단을 소리로 구분하라. rhythm이 없는 패턴은 메트로놈만으로도 완전히 동작해야 한다.
...
| 빔 1개 | 8분음표 | `subdivision:2` 격자 | — |
| 빔 2개 | 16분음표 | `subdivision:4` 격자 | — |
| 점(`.`) | 점음표(1.5배) | `durationSteps` 조정 | 더 길게 |
| 곡선(`⌒`) | **붙임줄(tie)** | `tie:true` | 새로 안 침, 지속 |
| 쉼표 기호 | 쉼 | `dir:'rest'` | 무음 |
| ×(엑스) 머리 | 뮤트/고스트 | `dir:'mute'` | 짧고 둔탁 |
| `║: … :║` | 반복기호 | `bars`·loop로 처리 | 반복 재생 |
| `3/4·4/4·6/8·12/8` | 박자표 | `timeSignature`,`beatsPerBar` | 마디 길이 |
| 셔플 “길게-짧게” | 스윙 | `feelTiming:'swing'`,`swing≈0.66` | 뒷박 지연 |
...
> 합계 76개. 파일명 끝의 `Ex01`/`EX01` 번호가 `exerciseNo`이며, 표시 제목은 `"{카테고리 라벨} Ex-{exerciseNo}"`.
...
[파일명 → exerciseNo/대상명]
  정규식으로 끝의 (?:Ex|EX)\s*0*(\d+) 를 뽑아 number → exerciseNo.
  대상 파일: public/strums/{슬러그}/ex{2자리 0패딩}.png   예) ex01.png, ex12.png
  patternId = `{categoryId}_ex{2자리}`                     예) country_ex01
  displayNo = `Ex{2자리}`                                  예) "Ex01"
  title     = `{카테고리 labelEn} Ex-{exerciseNo}`         예) "Country Ex-1"
  (rhythm 키는 patternId와 동일하게: strumRhythms['country_ex01'])
```

---

### 마지막 한마디 (AI에게)
> “Core(이미지 뷰어)를 먼저 완성해 ‘정확히 보여주는’ 토대를 만들고, 그 위에 Upgrade(오디오 재생 + 동기 플레이헤드 + 기타/우쿨렐레 음색)를 얹어 ‘듣고 따라 치는’ 연습 도구로 끌어올려라. 표시는 원본 PNG, 소리·커서는 리듬 데이터 — 둘을 분리하면 안전하고 강력하다. 영상/자매 앱의 디자인 톤(밝은 배경·부드러운 그림자·정돈된 카드·알록달록 카테고리 색·연핑크 로고)을 일관되게 재현하고, TypeScript 타입 안정성과 `npm run prepare:strums`·`npm run build` 성공까지 끝내라.”
